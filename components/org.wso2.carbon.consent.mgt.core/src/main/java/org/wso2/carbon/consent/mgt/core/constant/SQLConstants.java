...
public class SQLConstants {
    ...
    // V2 API: UUID-aware list queries for purposes (include UUID and LATEST_VERSION_ID columns).
    public static final String LIST_PAGINATED_PURPOSE_MYSQL_WITH_UUID = "SELECT ID, NAME, DESCRIPTION, " +
            "PURPOSE_GROUP, GROUP_TYPE, TENANT_ID, UUID FROM CM_PURPOSE " +
            "WHERE TENANT_ID = ? AND PURPOSE_GROUP LIKE ? AND " +
            "GROUP_TYPE LIKE ? ORDER BY ID ASC LIMIT ? OFFSET ?";

    public static final String LIST_PAGINATED_PURPOSE_DB2_WITH_UUID = "SELECT ID, NAME, DESCRIPTION, " +
            "PURPOSE_GROUP, GROUP_TYPE, TENANT_ID, UUID FROM (SELECT ROW_NUMBER() OVER " +
            "(ORDER BY ID) AS rn, p.* FROM CM_PURPOSE AS p) WHERE " +
            "TENANT_ID =? AND PURPOSE_GROUP LIKE ? AND GROUP_TYPE " +
            "LIKE ? AND rn BETWEEN ? AND ?";

    public static final String LIST_PAGINATED_PURPOSE_ORACLE_WITH_UUID = "SELECT ID, NAME, DESCRIPTION, " +
            "PURPOSE_GROUP, GROUP_TYPE, TENANT_ID, UUID FROM (SELECT ID, " +
            "NAME, DESCRIPTION, PURPOSE_GROUP, GROUP_TYPE, " +
            "TENANT_ID, UUID, rownum AS rnum FROM " +
            "(SELECT ID, NAME, DESCRIPTION, PURPOSE_GROUP, " +
            "GROUP_TYPE, TENANT_ID, UUID FROM CM_PURPOSE " +
            "ORDER BY ID) WHERE TENANT_ID =? AND PURPOSE_GROUP" +
            " LIKE ? AND GROUP_TYPE LIKE ? AND rownum <= ?) WHERE " +
            "rnum > ?";

    public static final String LIST_PAGINATED_PURPOSE_MSSQL_WITH_UUID = "SELECT ID, NAME, DESCRIPTION, " +
            "PURPOSE_GROUP, GROUP_TYPE, TENANT_ID, UUID FROM (SELECT ID, NAME," +
            " DESCRIPTION, PURPOSE_GROUP, GROUP_TYPE, TENANT_ID, UUID," +
            " ROW_NUMBER() OVER (ORDER" +
            " BY ID) AS RowNum FROM CM_PURPOSE) AS P WHERE P" +
            ".TENANT_ID = ? AND PURPOSE_GROUP LIKE ? AND GROUP_TYPE" +
            " LIKE ? AND P.RowNum BETWEEN ? AND ?";
    ...
    // V2 API: UUID and LATEST_VERSION_ID aware SQL (new columns added in V2 schema).
    ...
    public static final String GET_MAX_PURPOSE_VERSION_SQL = "SELECT MAX(VERSION) FROM CM_PURPOSE_VERSION " +
            "WHERE PURPOSE_ID = ?";

    public static final String DELETE_PURPOSE_VERSION_SQL = "DELETE FROM CM_PURPOSE_VERSION WHERE UUID = ?";

    // ---- Receipt search queries with purpose-ID filter ----
    // Variant: with SP tenant + with principal tenant
    public static final String SEARCH_RECEIPT_WITH_PURPOSE_SQL = "SELECT R.CONSENT_RECEIPT_ID, R.LANGUAGE, R" +
            ".PII_PRINCIPAL_ID, R.PRINCIPAL_TENANT_ID, R.STATE,RS.SP_DISPLAY_NAME,RS.SP_DESCRIPTION FROM CM_RECEIPT R" +
            " INNER JOIN CM_RECEIPT_SP_ASSOC RS ON R.CONSENT_RECEIPT_ID=RS.CONSENT_RECEIPT_ID INNER JOIN " +
            "CM_SP_PURPOSE_ASSOC SPA ON RS.ID = SPA.RECEIPT_SP_ASSOC WHERE PII_PRINCIPAL_ID LIKE ? AND " +
            "PRINCIPAL_TENANT_ID=? AND SP_NAME LIKE ? AND SP_TENANT_ID = ? AND STATE LIKE ? AND SPA.PURPOSE_ID = ? " +
            "ORDER BY RS.ID ASC LIMIT ? OFFSET ?";

    public static final String SEARCH_RECEIPT_WITH_PURPOSE_SQL_DB2 = "SELECT CONSENT_RECEIPT_ID, LANGUAGE, " +
            "PII_PRINCIPAL_ID, PRINCIPAL_TENANT_ID,STATE,SP_DISPLAY_NAME,SP_DESCRIPTION FROM (SELECT R" +
            ".CONSENT_RECEIPT_ID, R.LANGUAGE, R.PII_PRINCIPAL_ID, R.PRINCIPAL_TENANT_ID, RS.SP_DISPLAY_NAME," +
            "RS.SP_DESCRIPTION,R.STATE,ROW_NUMBER() OVER(ORDER BY R.CONSENT_RECEIPT_ID) AS ROWNUMBER FROM CM_RECEIPT R" +
            " INNER JOIN CM_RECEIPT_SP_ASSOC RS ON R.CONSENT_RECEIPT_ID = RS.CONSENT_RECEIPT_ID INNER JOIN " +
            "CM_SP_PURPOSE_ASSOC SPA ON RS.ID = SPA.RECEIPT_SP_ASSOC WHERE R.PII_PRINCIPAL_ID LIKE ? AND " +
            "PRINCIPAL_TENANT_ID = ? AND RS.SP_NAME LIKE ? AND RS.SP_TENANT_ID LIKE ? AND R.STATE LIKE ? AND " +
            "SPA.PURPOSE_ID = ?) AS X WHERE ROWNUMBER BETWEEN ? AND ?";

    public static final String SEARCH_RECEIPT_WITH_PURPOSE_SQL_INFORMIX = "SELECT R.CONSENT_RECEIPT_ID, R.LANGUAGE, R" +
            ".PII_PRINCIPAL_ID, R.PRINCIPAL_TENANT_ID, R.STATE,RS.SP_DISPLAY_NAME,RS.SP_DESCRIPTION FROM CM_RECEIPT R" +
            " INNER JOIN CM_RECEIPT_SP_ASSOC RS ON R.CONSENT_RECEIPT_ID=RS.CONSENT_RECEIPT_ID INNER JOIN " +
            "CM_SP_PURPOSE_ASSOC SPA ON RS.ID = SPA.RECEIPT_SP_ASSOC WHERE PII_PRINCIPAL_ID LIKE ? AND " +
            "PRINCIPAL_TENANT_ID =? AND SP_NAME LIKE ? AND SP_TENANT_ID LIKE ? AND STATE LIKE ? AND " +
            "SPA.PURPOSE_ID = ? ORDER BY RS.ID ASC LIMIT ? OFFSET ?";

    public static final String SEARCH_RECEIPT_WITH_PURPOSE_SQL_ORACLE = "SELECT CONSENT_RECEIPT_ID, LANGUAGE," +
            "PII_PRINCIPAL_ID, PRINCIPAL_TENANT_ID, STATE,SP_DISPLAY_NAME,SP_DESCRIPTION FROM (SELECT ROWNUM RNUM,A" +
            ".* FROM(SELECT R.CONSENT_RECEIPT_ID, R.LANGUAGE,R.PII_PRINCIPAL_ID,R.PRINCIPAL_TENANT_ID, R.STATE,RS" +
            ".SP_DISPLAY_NAME,RS.SP_DESCRIPTION FROM CM_RECEIPT R INNER JOIN CM_RECEIPT_SP_ASSOC RS ON R" +
            ".CONSENT_RECEIPT_ID=RS.CONSENT_RECEIPT_ID INNER JOIN CM_SP_PURPOSE_ASSOC SPA ON RS.ID = " +
            "SPA.RECEIPT_SP_ASSOC WHERE PII_PRINCIPAL_ID LIKE ? AND PRINCIPAL_TENANT_ID = ? AND SP_NAME LIKE ? AND " +
            "SP_TENANT_ID LIKE ? AND STATE LIKE ? AND SPA.PURPOSE_ID = ? ORDER BY R.CONSENT_RECEIPT_ID) A WHERE " +
            "ROWNUM <= ?) WHERE RNUM > ?";

    public static final String SEARCH_RECEIPT_WITH_PURPOSE_SQL_MSSQL = "SELECT CONSENT_RECEIPT_ID, LANGUAGE, " +
            "PII_PRINCIPAL_ID, PRINCIPAL_TENANT_ID, STATE, SP_DISPLAY_NAME, SP_DESCRIPTION FROM (SELECT R" +
            ".CONSENT_RECEIPT_ID, R.LANGUAGE, R.PII_PRINCIPAL_ID, R.PRINCIPAL_TENANT_ID, R.STATE, RS.SP_DISPLAY_NAME," +
            " RS.SP_DESCRIPTION, ROW_NUMBER() OVER (ORDER BY R.CONSENT_RECEIPT_ID) AS ROWNUM FROM CM_RECEIPT R INNER " +
            "JOIN CM_RECEIPT_SP_ASSOC RS ON R.CONSENT_RECEIPT_ID = RS.CONSENT_RECEIPT_ID INNER JOIN " +
            "CM_SP_PURPOSE_ASSOC SPA ON RS.ID = SPA.RECEIPT_SP_ASSOC WHERE PII_PRINCIPAL_ID LIKE ? AND " +
            "PRINCIPAL_TENANT_ID = ? AND SP_NAME LIKE ? AND SP_TENANT_ID LIKE ? AND STATE LIKE ? AND " +
            "SPA.PURPOSE_ID = ?) AS RES WHERE RES.ROWNUM BETWEEN ? AND ?";

    // Variant: without SP tenant + with principal tenant
    public static final String SEARCH_RECEIPT_WITHOUT_SP_TENANT_WITH_PURPOSE_SQL = "SELECT R.CONSENT_RECEIPT_ID, " +
            "R.LANGUAGE, R.PII_PRINCIPAL_ID, R.PRINCIPAL_TENANT_ID, R.STATE,RS.SP_DISPLAY_NAME,RS.SP_DESCRIPTION " +
            "FROM CM_RECEIPT R INNER JOIN CM_RECEIPT_SP_ASSOC RS ON R.CONSENT_RECEIPT_ID=RS.CONSENT_RECEIPT_ID INNER " +
            "JOIN CM_SP_PURPOSE_ASSOC SPA ON RS.ID = SPA.RECEIPT_SP_ASSOC WHERE PII_PRINCIPAL_ID LIKE ? AND " +
            "PRINCIPAL_TENANT_ID =? AND SP_NAME LIKE ? AND STATE LIKE ? AND SPA.PURPOSE_ID = ? ORDER BY RS.ID ASC " +
            "LIMIT ? OFFSET ?";

    public static final String SEARCH_RECEIPT_WITHOUT_SP_TENANT_WITH_PURPOSE_SQL_DB2 = "SELECT CONSENT_RECEIPT_ID, " +
            "LANGUAGE, PII_PRINCIPAL_ID, PRINCIPAL_TENANT_ID,STATE,SP_DISPLAY_NAME,SP_DESCRIPTION FROM (SELECT R" +
            ".CONSENT_RECEIPT_ID, R.LANGUAGE, R.PII_PRINCIPAL_ID, R.PRINCIPAL_TENANT_ID, R.STATE,SP_DISPLAY_NAME," +
            "SP_DESCRIPTION, ROW_NUMBER() OVER(ORDER BY R.CONSENT_RECEIPT_ID) AS ROWNUMBER FROM CM_RECEIPT R INNER " +
            "JOIN CM_RECEIPT_SP_ASSOC RS ON R.CONSENT_RECEIPT_ID = RS.CONSENT_RECEIPT_ID INNER JOIN " +
            "CM_SP_PURPOSE_ASSOC SPA ON RS.ID = SPA.RECEIPT_SP_ASSOC WHERE R.PII_PRINCIPAL_ID LIKE ? AND " +
            "PRINCIPAL_TENANT_ID=? AND RS.SP_NAME LIKE ? AND R.STATE LIKE ? AND SPA.PURPOSE_ID = ?) AS X WHERE " +
            "ROWNUMBER BETWEEN ? AND ?";

    public static final String SEARCH_RECEIPT_WITHOUT_SP_TENANT_WITH_PURPOSE_SQL_INFORMIX = "SELECT R" +
            ".CONSENT_RECEIPT_ID, R.LANGUAGE, R.PII_PRINCIPAL_ID, R.PRINCIPAL_TENANT_ID, R.STATE,RS.SP_DISPLAY_NAME," +
            "RS.SP_DESCRIPTION FROM CM_RECEIPT R INNER JOIN CM_RECEIPT_SP_ASSOC RS ON R" +
            ".CONSENT_RECEIPT_ID=RS.CONSENT_RECEIPT_ID INNER JOIN CM_SP_PURPOSE_ASSOC SPA ON RS.ID = " +
            "SPA.RECEIPT_SP_ASSOC WHERE PII_PRINCIPAL_ID LIKE ? AND PRINCIPAL_TENANT_ID = ? AND SP_NAME LIKE ? AND " +
            "STATE LIKE ? AND SPA.PURPOSE_ID = ? ORDER BY RS.ID ASC LIMIT ? OFFSET ?";

    public static final String SEARCH_RECEIPT_WITHOUT_SP_TENANT_WITH_PURPOSE_SQL_ORACLE = "SELECT CONSENT_RECEIPT_ID," +
            " LANGUAGE,PII_PRINCIPAL_ID, PRINCIPAL_TENANT_ID, STATE,SP_DISPLAY_NAME,SP_DESCRIPTION FROM (SELECT " +
            "ROWNUM RNUM,A.* FROM(SELECT R.CONSENT_RECEIPT_ID, R.LANGUAGE,R.PII_PRINCIPAL_ID,R.PRINCIPAL_TENANT_ID, " +
            "R.STATE,RS.SP_DISPLAY_NAME,RS.SP_DESCRIPTION FROM CM_RECEIPT R INNER JOIN CM_RECEIPT_SP_ASSOC RS ON R" +
            ".CONSENT_RECEIPT_ID=RS.CONSENT_RECEIPT_ID INNER JOIN CM_SP_PURPOSE_ASSOC SPA ON RS.ID = " +
            "SPA.RECEIPT_SP_ASSOC WHERE PII_PRINCIPAL_ID LIKE ? AND PRINCIPAL_TENANT_ID = ? AND SP_NAME LIKE ? AND " +
            "STATE LIKE ? AND SPA.PURPOSE_ID = ? ORDER BY R.CONSENT_RECEIPT_ID) A WHERE ROWNUM <= ?) WHERE RNUM > ?";

    public static final String SEARCH_RECEIPT_WITHOUT_SP_TENANT_WITH_PURPOSE_SQL_MSSQL = "SELECT CONSENT_RECEIPT_ID, " +
            "LANGUAGE, PII_PRINCIPAL_ID, PRINCIPAL_TENANT_ID, STATE, SP_DISPLAY_NAME, SP_DESCRIPTION FROM (SELECT R" +
            ".CONSENT_RECEIPT_ID, R.LANGUAGE, R.PII_PRINCIPAL_ID, R.PRINCIPAL_TENANT_ID, R.STATE, RS.SP_DISPLAY_NAME," +
            " RS.SP_DESCRIPTION, ROW_NUMBER() OVER (ORDER BY R.CONSENT_RECEIPT_ID) AS ROWNUM FROM CM_RECEIPT R INNER " +
            "JOIN CM_RECEIPT_SP_ASSOC RS ON R.CONSENT_RECEIPT_ID = RS.CONSENT_RECEIPT_ID INNER JOIN " +
            "CM_SP_PURPOSE_ASSOC SPA ON RS.ID = SPA.RECEIPT_SP_ASSOC WHERE PII_PRINCIPAL_ID LIKE ? AND " +
            "PRINCIPAL_TENANT_ID=? AND SP_NAME LIKE ? AND STATE LIKE ? AND SPA.PURPOSE_ID = ?) AS RES WHERE " +
            "RES.ROWNUM BETWEEN ? AND ?";

    // Variant: with SP tenant + without principal tenant
    public static final String SEARCH_RECEIPT_WITHOUT_PRINCIPLE_TENANT_WITH_PURPOSE_SQL = "SELECT R" +
            ".CONSENT_RECEIPT_ID, R.LANGUAGE, R.PII_PRINCIPAL_ID, R.PRINCIPAL_TENANT_ID, R.STATE,RS.SP_DISPLAY_NAME," +
            "RS.SP_DESCRIPTION FROM CM_RECEIPT R INNER JOIN CM_RECEIPT_SP_ASSOC RS ON R" +
            ".CONSENT_RECEIPT_ID=RS.CONSENT_RECEIPT_ID INNER JOIN CM_SP_PURPOSE_ASSOC SPA ON RS.ID = " +
            "SPA.RECEIPT_SP_ASSOC WHERE PII_PRINCIPAL_ID LIKE ? AND SP_NAME LIKE ? AND SP_TENANT_ID = ? AND STATE " +
            "LIKE ? AND SPA.PURPOSE_ID = ? ORDER BY RS.ID ASC LIMIT ? OFFSET ?";

    public static final String SEARCH_RECEIPT_WITHOUT_PRINCIPLE_TENANT_WITH_PURPOSE_SQL_DB2 = "SELECT " +
            "CONSENT_RECEIPT_ID, LANGUAGE, PII_PRINCIPAL_ID, PRINCIPAL_TENANT_ID,STATE,SP_DISPLAY_NAME,SP_DESCRIPTION" +
            " FROM (SELECT R.CONSENT_RECEIPT_ID, R.LANGUAGE, R.PII_PRINCIPAL_ID, R.PRINCIPAL_TENANT_ID, RS" +
            ".SP_DISPLAY_NAME,RS.SP_DESCRIPTION,R.STATE,ROW_NUMBER() OVER(ORDER BY R.CONSENT_RECEIPT_ID) AS ROWNUMBER" +
            " FROM CM_RECEIPT R INNER JOIN CM_RECEIPT_SP_ASSOC RS ON R.CONSENT_RECEIPT_ID = RS.CONSENT_RECEIPT_ID " +
            "INNER JOIN CM_SP_PURPOSE_ASSOC SPA ON RS.ID = SPA.RECEIPT_SP_ASSOC WHERE R.PII_PRINCIPAL_ID LIKE ? AND " +
            "RS.SP_NAME LIKE ? AND RS.SP_TENANT_ID LIKE ? AND R.STATE LIKE ? AND SPA.PURPOSE_ID = ?) AS X WHERE " +
            "ROWNUMBER BETWEEN ? AND ?";

    public static final String SEARCH_RECEIPT_WITHOUT_PRINCIPLE_TENANT_WITH_PURPOSE_SQL_INFORMIX = "SELECT R" +
            ".CONSENT_RECEIPT_ID, R.LANGUAGE, R.PII_PRINCIPAL_ID, R.PRINCIPAL_TENANT_ID, R.STATE,RS.SP_DISPLAY_NAME," +
            "RS.SP_DESCRIPTION FROM CM_RECEIPT R INNER JOIN CM_RECEIPT_SP_ASSOC RS ON R" +
            ".CONSENT_RECEIPT_ID=RS.CONSENT_RECEIPT_ID INNER JOIN CM_SP_PURPOSE_ASSOC SPA ON RS.ID = " +
            "SPA.RECEIPT_SP_ASSOC WHERE PII_PRINCIPAL_ID LIKE ? AND SP_NAME LIKE ? AND SP_TENANT_ID LIKE ? AND STATE " +
            "LIKE ? AND SPA.PURPOSE_ID = ? ORDER BY RS.ID ASC LIMIT ? OFFSET ?";

    public static final String SEARCH_RECEIPT_WITHOUT_PRINCIPLE_TENANT_WITH_PURPOSE_SQL_ORACLE = "SELECT " +
            "CONSENT_RECEIPT_ID, LANGUAGE,PII_PRINCIPAL_ID, PRINCIPAL_TENANT_ID, STATE,SP_DISPLAY_NAME,SP_DESCRIPTION" +
            " FROM (SELECT ROWNUM RNUM,A.* FROM(SELECT R.CONSENT_RECEIPT_ID, R.LANGUAGE,R.PII_PRINCIPAL_ID," +
            "R.PRINCIPAL_TENANT_ID, R.STATE,RS.SP_DISPLAY_NAME,RS.SP_DESCRIPTION FROM CM_RECEIPT R INNER JOIN " +
            "CM_RECEIPT_SP_ASSOC RS ON R.CONSENT_RECEIPT_ID=RS.CONSENT_RECEIPT_ID INNER JOIN CM_SP_PURPOSE_ASSOC SPA " +
            "ON RS.ID = SPA.RECEIPT_SP_ASSOC WHERE PII_PRINCIPAL_ID LIKE ? AND SP_NAME LIKE ? AND SP_TENANT_ID LIKE ? " +
            "AND STATE LIKE ? AND SPA.PURPOSE_ID = ? ORDER BY R.CONSENT_RECEIPT_ID) A WHERE ROWNUM <= ?) WHERE RNUM > ?";

    public static final String SEARCH_RECEIPT_WITHOUT_PRINCIPLE_TENANT_WITH_PURPOSE_SQL_MSSQL = "SELECT " +
            "CONSENT_RECEIPT_ID, LANGUAGE, PII_PRINCIPAL_ID, PRINCIPAL_TENANT_ID, STATE, SP_DISPLAY_NAME, " +
            "SP_DESCRIPTION FROM (SELECT R.CONSENT_RECEIPT_ID, R.LANGUAGE, R.PII_PRINCIPAL_ID, R.PRINCIPAL_TENANT_ID," +
            " R.STATE, RS.SP_DISPLAY_NAME, RS.SP_DESCRIPTION, ROW_NUMBER() OVER (ORDER BY R.CONSENT_RECEIPT_ID) AS " +
            "ROWNUM FROM CM_RECEIPT R INNER JOIN CM_RECEIPT_SP_ASSOC RS ON R.CONSENT_RECEIPT_ID = RS.CONSENT_RECEIPT_ID" +
            " INNER JOIN CM_SP_PURPOSE_ASSOC SPA ON RS.ID = SPA.RECEIPT_SP_ASSOC WHERE PII_PRINCIPAL_ID LIKE ? AND " +
            "SP_NAME LIKE ? AND SP_TENANT_ID LIKE ? AND STATE LIKE ? AND SPA.PURPOSE_ID = ?) AS RES WHERE " +
            "RES.ROWNUM BETWEEN ? AND ?";

    // Variant: without SP tenant + without principal tenant
    public static final String SEARCH_RECEIPT_WITHOUT_SP_AND_PRINCIPLE_TENANT_WITH_PURPOSE_SQL = "SELECT R" +
            ".CONSENT_RECEIPT_ID, R.LANGUAGE, R.PII_PRINCIPAL_ID, R.PRINCIPAL_TENANT_ID, R.STATE,RS.SP_DISPLAY_NAME," +
            "RS.SP_DESCRIPTION FROM CM_RECEIPT R INNER JOIN CM_RECEIPT_SP_ASSOC RS ON R" +
            ".CONSENT_RECEIPT_ID=RS.CONSENT_RECEIPT_ID INNER JOIN CM_SP_PURPOSE_ASSOC SPA ON RS.ID = " +
            "SPA.RECEIPT_SP_ASSOC WHERE PII_PRINCIPAL_ID LIKE ? AND SP_NAME LIKE ? AND STATE LIKE ? AND " +
            "SPA.PURPOSE_ID = ? ORDER BY RS.ID ASC LIMIT ? OFFSET ?";

    public static final String SEARCH_RECEIPT_WITHOUT_SP_AND_PRINCIPLE_TENANT_WITH_PURPOSE_SQL_DB2 = "SELECT " +
            "CONSENT_RECEIPT_ID, LANGUAGE, PII_PRINCIPAL_ID, PRINCIPAL_TENANT_ID,STATE,SP_DISPLAY_NAME,SP_DESCRIPTION" +
            " FROM (SELECT R.CONSENT_RECEIPT_ID, R.LANGUAGE, R.PII_PRINCIPAL_ID, R.PRINCIPAL_TENANT_ID, RS" +
            ".SP_DISPLAY_NAME,RS.SP_DESCRIPTION,R.STATE,ROW_NUMBER() OVER(ORDER BY R.CONSENT_RECEIPT_ID) AS ROWNUMBER" +
            " FROM CM_RECEIPT R INNER JOIN CM_RECEIPT_SP_ASSOC RS ON R.CONSENT_RECEIPT_ID = RS.CONSENT_RECEIPT_ID " +
            "INNER JOIN CM_SP_PURPOSE_ASSOC SPA ON RS.ID = SPA.RECEIPT_SP_ASSOC WHERE R.PII_PRINCIPAL_ID LIKE ? AND " +
            "RS.SP_NAME LIKE ? AND R.STATE LIKE ? AND SPA.PURPOSE_ID = ?) AS X WHERE ROWNUMBER BETWEEN ? AND ?";

    public static final String SEARCH_RECEIPT_WITHOUT_SP_AND_PRINCIPLE_TENANT_WITH_PURPOSE_SQL_INFORMIX = "SELECT R" +
            ".CONSENT_RECEIPT_ID, R.LANGUAGE, R.PII_PRINCIPAL_ID, R.PRINCIPAL_TENANT_ID, R.STATE,RS.SP_DISPLAY_NAME," +
            "RS.SP_DESCRIPTION FROM CM_RECEIPT R INNER JOIN CM_RECEIPT_SP_ASSOC RS ON R" +
            ".CONSENT_RECEIPT_ID=RS.CONSENT_RECEIPT_ID INNER JOIN CM_SP_PURPOSE_ASSOC SPA ON RS.ID = " +
            "SPA.RECEIPT_SP_ASSOC WHERE PII_PRINCIPAL_ID LIKE ? AND SP_NAME LIKE ? AND STATE LIKE ? AND " +
    ...
    public static final String SEARCH_RECEIPT_WITHOUT_SP_AND_PRINCIPLE_TENANT_WITH_PURPOSE_SQL_ORACLE = "SELECT " +
            "CONSENT_RECEIPT_ID, LANGUAGE,PII_PRINCIPAL_ID, PRINCIPAL_TENANT_ID, STATE,SP_DISPLAY_NAME,SP_DESCRIPTION" +
            " FROM (SELECT ROWNUM RNUM,A.* FROM(SELECT R.CONSENT_RECEIPT_ID, R.LANGUAGE,R.PII_PRINCIPAL_ID," +
            "R.PRINCIPAL_TENANT_ID, R.STATE,RS.SP_DISPLAY_NAME,RS.SP_DESCRIPTION FROM CM_RECEIPT R INNER JOIN " +
            "CM_RECEIPT_SP_ASSOC RS ON R.CONSENT_RECEIPT_ID=RS.CONSENT_RECEIPT_ID INNER JOIN CM_SP_PURPOSE_ASSOC SPA " +
            "ON RS.ID = SPA.RECEIPT_SP_ASSOC WHERE PII_PRINCIPAL_ID LIKE ? AND SP_NAME LIKE ? AND STATE LIKE ? AND " +
            "SPA.PURPOSE_ID = ? ORDER BY R.CONSENT_RECEIPT_ID) A WHERE ROWNUM <= ?) WHERE RNUM > ?";

    public static final String SEARCH_RECEIPT_WITHOUT_SP_AND_PRINCIPLE_TENANT_WITH_PURPOSE_SQL_MSSQL = "SELECT " +
            "CONSENT_RECEIPT_ID, LANGUAGE, PII_PRINCIPAL_ID, PRINCIPAL_TENANT_ID, STATE, SP_DISPLAY_NAME, " +
            "SP_DESCRIPTION FROM (SELECT R.CONSENT_RECEIPT_ID, R.LANGUAGE, R.PII_PRINCIPAL_ID, R.PRINCIPAL_TENANT_ID," +
            " R.STATE, RS.SP_DISPLAY_NAME, RS.SP_DESCRIPTION, ROW_NUMBER() OVER (ORDER BY R.CONSENT_RECEIPT_ID) AS " +
            "ROWNUM FROM CM_RECEIPT R INNER JOIN CM_RECEIPT_SP_ASSOC RS ON R.CONSENT_RECEIPT_ID = RS.CONSENT_RECEIPT_ID" +
            " INNER JOIN CM_SP_PURPOSE_ASSOC SPA ON RS.ID = SPA.RECEIPT_SP_ASSOC WHERE PII_PRINCIPAL_ID LIKE ? AND " +
            "SP_NAME LIKE ? AND STATE LIKE ? AND SPA.PURPOSE_ID = ?) AS RES WHERE RES.ROWNUM BETWEEN ? AND ?";

    // ---- V2 API: INSERT with VALIDITY_TIME ----
    ...
    // ---- V2 API: GET_RECEIPT_SQL with VALIDITY_TIME ----
    ...
    // ---- V2 API: CM_CONSENT_AUTHORIZATION CRUD ----
    ...
    // ---- V2 API: purpose list with name filter ----
    public static final String LIST_PAGINATED_PURPOSE_WITH_NAME_MYSQL = "SELECT ID, NAME, DESCRIPTION, " +
            "PURPOSE_GROUP, GROUP_TYPE, TENANT_ID, UUID, LATEST_VERSION_ID FROM CM_PURPOSE " +
            "WHERE TENANT_ID = ? AND PURPOSE_GROUP LIKE ? AND " +
            "GROUP_TYPE LIKE ? AND LOWER(NAME) LIKE LOWER(?) ORDER BY ID ASC LIMIT ? OFFSET ?";

    public static final String LIST_PAGINATED_PURPOSE_WITH_NAME_DB2 = "SELECT ID, NAME, DESCRIPTION, " +
            "PURPOSE_GROUP, GROUP_TYPE, TENANT_ID, UUID, LATEST_VERSION_ID FROM (SELECT ROW_NUMBER() OVER " +
            "(ORDER BY ID) AS rn, p.* FROM CM_PURPOSE AS p) WHERE " +
            "TENANT_ID =? AND PURPOSE_GROUP LIKE ? AND GROUP_TYPE LIKE ? AND LOWER(NAME) LIKE LOWER(?) " +
            "AND rn BETWEEN ? AND ?";

    public static final String LIST_PAGINATED_PURPOSE_WITH_NAME_ORACLE = "SELECT ID, NAME, DESCRIPTION, " +
            ...
            "ORDER BY ID) WHERE TENANT_ID =? AND PURPOSE_GROUP LIKE ? AND GROUP_TYPE LIKE ? " +
            "AND LOWER(NAME) LIKE LOWER(?) AND rownum <= ?) WHERE rnum > ?";

    public static final String LIST_PAGINATED_PURPOSE_WITH_NAME_MSSQL = "SELECT ID, NAME, DESCRIPTION, " +
            ...
            " ROW_NUMBER() OVER (ORDER BY ID) AS RowNum FROM CM_PURPOSE) AS P WHERE P" +
            ".TENANT_ID = ? AND PURPOSE_GROUP LIKE ? AND GROUP_TYPE LIKE ? AND LOWER(P.NAME) LIKE LOWER(?) " +
            "AND P.RowNum BETWEEN ? AND ?";

    // ---- V2 API: PII category list with name filter ----
    public static final String LIST_PAGINATED_PII_CATEGORY_WITH_NAME_MYSQL = "SELECT ID, NAME, DESCRIPTION, " +
            "IS_SENSITIVE, TENANT_ID, DISPLAY_NAME, UUID FROM CM_PII_CATEGORY WHERE TENANT_ID = ? " +
            "AND LOWER(NAME) LIKE LOWER(?) ORDER BY ID ASC LIMIT ? OFFSET ?";

    public static final String LIST_PAGINATED_PII_CATEGORY_WITH_NAME_DB2 = "SELECT ID, NAME, DESCRIPTION, " +
            "IS_SENSITIVE, TENANT_ID, DISPLAY_NAME, UUID FROM (SELECT ROW_NUMBER() OVER (ORDER BY ID) AS rn, p.*  " +
            "FROM CM_PII_CATEGORY AS p) WHERE TENANT_ID = ? AND LOWER(NAME) LIKE LOWER(?) AND rn BETWEEN ? AND ?";

    public static final String LIST_PAGINATED_PII_CATEGORY_WITH_NAME_MSSQL = "SELECT ID, NAME, DESCRIPTION, " +
            "IS_SENSITIVE, TENANT_ID, DISPLAY_NAME, UUID FROM " +
            "(SELECT ID, NAME, DESCRIPTION, IS_SENSITIVE, TENANT_ID, DISPLAY_NAME, UUID, " +
            "ROW_NUMBER() OVER (ORDER BY ID) AS RowNum FROM CM_PII_CATEGORY) AS P " +
            "WHERE P.TENANT_ID = ? AND LOWER(P.NAME) LIKE LOWER(?) AND P.RowNum BETWEEN ? AND ?";

    public static final String LIST_PAGINATED_PII_CATEGORY_WITH_NAME_ORACLE = "SELECT ID, NAME, DESCRIPTION, " +
            "IS_SENSITIVE, TENANT_ID, DISPLAY_NAME, UUID FROM (SELECT ID, NAME, DESCRIPTION, IS_SENSITIVE, " +
            "TENANT_ID, DISPLAY_NAME, UUID, rownum AS rnum FROM (SELECT ID, NAME, DESCRIPTION, IS_SENSITIVE, " +
            "TENANT_ID, DISPLAY_NAME, UUID FROM CM_PII_CATEGORY ORDER BY ID) " +
            "WHERE TENANT_ID = ? AND LOWER(NAME) LIKE LOWER(?) AND rownum <= ?) WHERE rnum > ?";
}
