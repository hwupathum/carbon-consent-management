...
public class ConsentReceiptsService {
    ...
    public ConsentResponseDTO createConsent(ConsentCreateRequest request) throws ConsentManagementException {
        ...
        // No authorizations → ACTIVE; has authorizations → PENDING
        boolean hasPendingAuth = request.getAuthorizations() != null && !request.getAuthorizations().isEmpty();
        responseDTO.setState(hasPendingAuth ? ConsentResponseDTO.StateEnum.PENDING : ConsentResponseDTO.StateEnum.ACTIVE);
        return responseDTO;
    }
    ...
    public Response listConsents(String subjectId, String serviceId, String state,
        ...
        if (purposeIdParam != null) {
            Purpose purpose = consentManager.getPurposeByUuid(purposeIdParam.toString());
            receipts = consentManager.searchReceipts(limit, offset, subjectId, null, serviceId, state,
                    purpose.getId());
        } else {
            receipts = consentManager.searchReceipts(limit, offset, subjectId, null, serviceId, state);
        }
    ...
    private ReceiptInput buildReceiptInput(ConsentCreateRequest request, int defaultPurposeCategoryId)
        ...
        if (request.getProperties() != null) {
            ...
            for (KeyValuePair kv : request.getProperties()) {
